...
StateNotifierProvider<JobsFilterNotifier, JobsFilterState>((ref) {
  return JobsFilterNotifier();
});

final jobCategoriesProvider = Provider<AsyncValue<List<String>>>((ref) {
...
final filteredJobsProvider = Provider<AsyncValue<List<JobModel>>>((ref) {
  ...
  return jobsAsync.whenData((jobs) {
    return jobs.where((job) {
      if (filter.onlyOpenJobs && job.status != JobStatus.open) {
        return false;
      }
      ...
      if (filter.minBudget != null && job.budget < filter.minBudget!) {
        return false;
      }

      if (filter.maxBudget != null && job.budget > filter.maxBudget!) {
        return false;
      }
